...
# Dicionários de mapeamento para as opções de Radio Buttons
opcoes_organizacao = {
    "Nada organizado": "0",
    "Pouco": "1",
    "Moderado": "2",
...
st.title("🎓 Sistema de Acompanhamento da Pós-Graduação")
st.write("Por favor, navegue pelas abas abaixo e preencha todos os blocos necessários.")

# Criação das 3 abas visuais do Streamlit
...
# Formulário único englobando todas as abas
with st.form("form_redcap_abas", clear_on_submit=False):
    ...
    with tab_pessoais:
        ...
        ra = st.text_input("RA (Registro Acadêmico) *", help="Insira apenas números inteiros.")
        nome = st.text_input("Nome Completo *")
        e_mail = st.text_input("E-mail Institucional *")
        
    # -------------------------------------------------------------------------
    # ABA 2: CHECK IN PROGRESSO POS GRADUACAO
    # -------------------------------------------------------------------------
    with tab_progresso:
        st.header("📊 Check-In Progresso Pós-Graduação")
        
        # Tratamento de Data/Horário automático conforme metadados (@TODAY / @NOW)
        data_envio_redcap = datetime.now().strftime("%Y-%m-%d")
        data_visualizacao = datetime.now().strftime("%d/%m/%Y")
        horario_atual = datetime.now().strftime("%H:%M")
        
        st.info(f"📅 **Data de registro:** {data_visualizacao} | ⏰ **Horário:** {horario_atual}")
        st.markdown("---")
        
        st.subheader("BLOCO 1 - Andamento Acadêmico")
        andamento_academico = st.select_slider("Andamento Acadêmico *", options=opcoes_progresso, key="s_andamento")
        
        # Upload obrigatório do arquivo do cronograma
        arquivo_cronograma = st.file_uploader("Envie aqui seu cronograma (PDF, DOCX, etc.) *", type=["pdf", "docx", "xlsx", "txt"])
        
        disciplinas_obrigatorias = st.select_slider("Disciplinas obrigatórias *", options=opcoes_progresso, key="s_disc")
        desenvolvimento_do_projeto = st.select_slider("Desenvolvimento do projeto/pesquisa *", options=opcoes_progresso, key="s_desenv")
        revisao_de_literatura = st.select_slider("Revisão de literatura *", options=opcoes_progresso, key="s_rev")
        coleta_de_dados = st.select_slider("Coleta de dados *", options=opcoes_progresso, key="s_coleta")
        analise_de_dados = st.select_slider("Análise de dados *", options=opcoes_progresso, key="s_analise")
        escrita_cientifica = st.select_slider("Escrita científica (artigo/dissertação/tese) *", options=opcoes_progresso, key="s_escrita")
    ...
    with tab_sentimentos:
        ...
        # Inicialização preventiva de dados condicionais
        descreva_como_se_sente = ""
        alguns_sinais_de_estresse = ""
        check_choices = [False] * 20
        
        # Lógica Condicional Estrita ([por_que_se_sente_bem] == '1')
        if por_que_se_sente_bem == "Sim":
            st.markdown("#### Detalhes sobre seus sentimentos")
            descreva_como_se_sente = st.text_area("Descreva como se sente *")
    ...
    # Centralização do botão de envio único posicionado fora/abaixo das abas
    submetido = st.form_submit_button("Submeter Formulário Completo")

# Processamento da lógica de envio
if submetido:
    if not ra or not nome or not e_mail:
        st.error("Por favor, preencha a aba **1. Dados Pessoais** por completo.")
    elif arquivo_cronograma is None:
        st.error("O upload do **cronograma** na aba **2. Progresso Acadêmico** é obrigatório.")
    else:
